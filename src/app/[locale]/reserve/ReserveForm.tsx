...
export default function ReserveForm({ locale, whatsapp, menuItems }: Props) {
    ...
    const cartItems: CartItem[] = useMemo(
        () =>
            menuItems.filter((m) => (cart[m.id] ?? 0) > 0).map((m) => ({ ...m, qty: cart[m.id] })),
        [cart, menuItems],
    );
    ...
    function buildMessage() {
        ...
        if (isBn) {
            ...
            if (cartItems.length) {
                lines.push("অর্ডার:");
                cartItems.forEach((i) =>
                    lines.push(`  • ${i.nameBn} × ${i.qty} = ৳${i.price * i.qty}`),
                );
            ...
            if (cartItems.length) {
                lines.push("Order:");
                cartItems.forEach((i) =>
                    lines.push(`  • ${i.nameEn} × ${i.qty} = ৳${i.price * i.qty}`),
                );
    ...
    const inp =
        "w-full border border-[var(--color-earth-100)] bg-white px-4 py-3 text-sm text-[var(--color-ink)] placeholder:text-[var(--color-ink)]/30 focus:outline-none focus:border-[var(--color-terracotta-400)] transition";
    const label =
    ...
    return (
        <form
            ...
            className="space-y-5 border border-(--color-earth-100) bg-white p-6 sm:p-8"
        >
            {/* ── Order items ── */}
            <div>
                <div className="flex items-center justify-between mb-3">
                    <span className={label} style={{ marginBottom: 0 }}>
                        ...
                        <span aria-hidden className="text-(--color-terracotta-500)">
                            *
                        </span>
                    </span>
                    {totalQty > 0 && (
                        <span className="text-xs font-semibold text-(--color-terracotta-600)">
                            {totalQty} {isBn ? "টি" : totalQty === 1 ? "item" : "items"} · ৳
                            {totalPrice}
                        </span>
                    )}
                </div>
                ...
                {/* Cart */}
                {cartItems.length > 0 ? (
                    <div className="border border-(--color-earth-100) divide-y divide-(--color-earth-100) mb-3">
                        {cartItems.map((item) => (
                            <div key={item.id} className="flex items-center gap-3 px-4 py-3">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-[11px] text-(--color-ink)/40 mt-0.5">
                                ...
                                <div className="flex items-center gap-1.5 shrink-0">
                                    <button
                                        ...
                                        className="w-7 h-7 flex items-center justify-center border border-(--color-earth-200) hover:border-(--color-terracotta-400) text-(--color-ink)/50 hover:text-(--color-terracotta-600) transition-colors"
                                    >
                                        {item.qty === 1 ? (
                                    ...
                                    <span className="w-6 text-center text-sm font-semibold text-(--color-ink)">
                                        {item.qty}
                                    </span>
                                    <button
                                        type="button"
                                ...
                                <span className="text-sm font-semibold text-(--color-terracotta-600) w-14 text-right shrink-0">
                                    ৳{item.price * item.qty}
                                </span>
                            </div>
                        ))}
                        {totalQty > 0 && (
                            <div className="flex justify-between items-center px-4 py-2.5 bg-(--color-earth-50)">
                                <span className="text-xs font-semibold text-(--color-ink)/50 uppercase tracking-widest">
                                    {isBn ? "মোট" : "Total"}
                                </span>
                    ...
                    <div className="border border-dashed border-(--color-earth-200) px-4 py-6 text-center text-sm text-(--color-ink)/35 mb-3">
                ...
                <button
                    ...
                    className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-widest text-(--color-terracotta-500) hover:text-(--color-terracotta-700) transition-colors"
                >
                ...
                {showPicker && (
                    <div className="mt-3 border border-(--color-earth-100) bg-(--color-earth-50)">
                        {/* Category tabs */}
                        <div className="flex overflow-x-auto border-b border-(--color-earth-100) scrollbar-hide">
                                ...
                                return (
                                    <button
                                        ...
                                        className={`shrink-0 px-4 py-2.5 text-[10px] font-semibold uppercase tracking-widest transition-colors whitespace-nowrap ${
                                            pickerCat === cat
                                                ? "bg-white text-(--color-ink) border-b-2 border-(--color-terracotta-500) -mb-px"
                                                : "text-(--color-ink)/40 hover:text-(--color-ink)"
                                        }`}
                                    >
                        ...
                        <div className="max-h-60 overflow-y-auto divide-y divide-(--color-earth-100)">
                            {pickerItems.map((item) => {
                                const qty = cart[item.id] ?? 0;
                                return (
                                    <div
                                        key={item.id}
                                        className="flex items-center gap-3 px-4 py-2.5 bg-white hover:bg-(--color-cream) transition-colors"
                                    >
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm text-(--color-ink) leading-tight">
                                                {isBn ? item.nameBn : item.nameEn}
                                            </p>
                                            <p className="text-[11px] text-(--color-ink)/40">
                                                ৳{item.price}
                                            </p>
                                        </div>
                                        {qty === 0 ? (
                                            <button
                                                type="button"
                                                onClick={() => setQty(item.id, 1)}
                                                className="shrink-0 flex items-center gap-1 px-3 py-1.5 border border-(--color-earth-200) hover:border-(--color-terracotta-400) text-(--color-ink)/50 hover:text-(--color-terracotta-600) text-[11px] font-medium transition-colors"
                                            >
                                            ...
                                            <div className="flex items-center gap-1.5 shrink-0">
                                                <button
                                                    type="button"
                                                    onClick={() => setQty(item.id, -1)}
                                                    className="w-6 h-6 flex items-center justify-center border border-(--color-earth-200) text-(--color-ink)/50 hover:text-(--color-terracotta-600) transition-colors"
                                                >
                                                    <MinusIcon size={10} strokeWidth={2.5} />
                                                </button>
                                                <span className="w-5 text-center text-sm font-semibold text-(--color-ink)">
                                                    {qty}
                                                </span>
                                                <button
                                                    type="button"
            ...
            {/* ── Party size ── */}
            <div>
                <label htmlFor="reserve-party" className={label}>
                    {t("partySizeLabel")}
                </label>
                <input
                    id="reserve-party"
