...
export default function DateTimePicker({ value, onChange, locale }: Props) {
    ...
    const periodBn =
        currentH < 12 ? "সকাল" : currentH < 17 ? "বিকেল" : currentH < 20 ? "সন্ধ্যা" : "রাত";

    const incH = () => {
        if (currentH < CLOSE_H) setTime(currentH + 1, currentM);
    };
    const decH = () => {
        if (currentH > OPEN_H) setTime(currentH - 1, currentM);
    };
    const incM = () => {
        const i = MINUTES.indexOf(currentM);
        if (i < MINUTES.length - 1) setTime(currentH, MINUTES[i + 1]);
    };
    const decM = () => {
        const i = MINUTES.indexOf(currentM);
        if (i > 0) setTime(currentH, MINUTES[i - 1]);
    };
    const togglePM = () => {
    ...
    const cal = {
        ...
        day_selected:
            "!bg-[var(--color-terracotta-500)] !text-white !font-semibold hover:!bg-[var(--color-terracotta-600)]",
        day_today:
    ...
    return (
        <div className="border border-(--color-earth-100) overflow-hidden">
            {/* ── Summary strip ── */}
            <div
                className={cn(
                    "flex items-center gap-3 px-4 py-3 text-sm font-medium transition-colors",
                    summaryDate || summaryTime
                        ? "bg-(--color-terracotta-500) text-white"
                        : "bg-(--color-earth-50) text-(--color-ink)/35",
                )}
            >
            ...
            <div className="flex flex-col sm:flex-row">
                ...
                {/* Divider */}
                <div className="hidden sm:block w-px bg-(--color-earth-100)" />
                <div className="block sm:hidden h-px bg-(--color-earth-100)" />

                {/* Time dialler — fixed width panel */}
                <div className="sm:w-44 flex flex-col items-center justify-center gap-4 p-5 bg-(--color-ink)/90">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="font-display text-3xl font-bold text-white/25 mt-6 select-none">
                            :
                        </span>
                        <Spinner
